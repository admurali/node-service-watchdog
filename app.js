        ...
        const requests = myCache.keys().map(item => new Promise((resolve) => {
          pidusage(item, (error, stats) => {
            if (error) {
              ...
              myCache.del(item, (cacheError, count) => {
                if (cacheError) {
                  console.log(`Unable to remove dead process from cache. Got error: ${error.message}`);
                } else if (count === 0) {
                  console.log('Unable to remove dead process from cache');
                } else {
                  console.log('Removed dead process from cache');
                }
              });
            } else {
              myCache.get(item, (cacheError, value) => {
                if (err) {
                  console.log(`Unable to get value based on cache Id Got error: ${error.message}`);
                  cache.push(stats);
                  resolve();
                } else if (value === undefined || !value.progress || !value.data) {
                  // console.log('Got undefined progress value based on cache Id');
                  cache.push(stats);
                  resolve();
                } else {
                  stats.progress = value.progress;
                  stats.data = value.data;
                  cache.push(stats);
                  resolve();
                }
              });
